...
const deadPixelFaqs = () => [
  {
    question: "What's the difference between a dead pixel and a stuck pixel?",
    answer: 'A dead pixel stays permanently off and shows as a black dot no matter what the screen displays, because the pixel receives no power at all. A stuck pixel is still powered but frozen on one color — usually red, green, or blue — so it stands out brightly against a dark background. The practical difference is that stuck pixels sometimes recover on their own or with gentle pressure and pixel-refresh software, while dead pixels almost never do.',
  },
  {
    question: 'How many colors should I actually test?',
    answer: 'All six, if you want a reliable answer. Black and white catch opposite problems, and red, green, and blue each isolate one sub-pixel channel, so a fault in a single channel can hide completely on the other five screens. Grey is the one most people skip, and it is the only screen that reliably exposes backlight unevenness. A full pass takes under a minute.',
  },
  {
    question: 'Can dead pixels be fixed?',
    answer: 'Usually not. A true dead pixel is a hardware fault in the panel, so no software can switch it back on. Stuck pixels are a different story — rapid color cycling or very gentle pressure on the area sometimes frees them, though neither is guaranteed. If a new display has more defects than the manufacturer allows, a warranty claim is the realistic fix, and a recording of this test makes useful evidence.',
  },
  {
    question: 'Does this test work on phone and tablet screens, not just monitors?',
    answer: 'Yes. The test runs in any browser and fills whatever display it is opened on, so phones, tablets, laptops, external monitors, and TVs all work the same way. On touch devices you can tap the next and previous controls or turn on auto-advance and simply watch. Clean the screen first — on a small, high-density panel, a speck of dust is easy to mistake for a dead pixel.',
  },
  {
    question: 'Is this test accurate for OLED displays too, or only LCD?',
    answer: 'It works on both, though the results read differently. On OLED, black switches pixels off completely, so any dot that still glows is genuinely stuck, and the black screen is the most informative of the six. Backlight bleeding does not exist on OLED because there is no backlight, so uneven grey usually points to panel wear or early burn-in instead. On LCD and LED panels, grey and white are where bleeding and clouding show up most clearly.',
  },
  {
    question: 'How long should I display each color?',
    answer: 'Around ten to fifteen seconds is enough to scan a screen properly, which is why auto-advance is set to five seconds as a starting pace rather than a limit. Move your eyes across the whole panel in sections instead of staring at the center, and check the corners and edges last, since that is where bleeding and clouding concentrate. Viewing from a slight angle on a second pass helps, because some defects only appear off-axis.',
  },
];
...
const burnInFaqs = () => [
  {
    question: 'What is OLED burn-in and what causes it?',
    answer: 'Burn-in is permanent or semi-permanent discoloration left behind when parts of an OLED panel age unevenly. Every OLED pixel dims slightly as it is used, so anything that stays in one place at high brightness — a status bar, a channel logo, a game HUD, a navigation row — wears its pixels faster than the moving content around them. Once the gap is wide enough, the outline of that static element stays faintly visible no matter what is on screen.',
  },
  {
    question: 'How can I tell if my screen already has burn-in?',
    answer: 'Use Static Check mode and step through the grey fills in a dimly lit room. An even mid-grey is the most revealing, because faint discoloration that white washes out and black hides becomes obvious against a flat mid-tone. Look for shadowy outlines of interface elements you use constantly, or patches that stay in the same place while the fill color changes around them. Anything that moves with the image is reflection, not burn-in.',
  },
  {
    question: 'Does running this test itself cause burn-in?',
    answer: 'Not in any realistic use. Burn-in comes from static content held for hundreds of hours, not from a check that lasts a minute or two, and Static Check is meant to be stepped through rather than parked on. Moving Pattern mode is the opposite of a risk: nothing in it stays still, so no pixel holds one value long enough to age unevenly. If you want to be careful, run the bright fills briefly and lower the brightness slider.',
  },
  {
    question: 'How is this different from a plain black or white screen test?',
    answer: 'A black screen is best for spotting stuck pixels and light leaking from the edges, and a white screen is best for dead pixels and dust. Both are single flat fills for finding defects. This page targets burn-in specifically: it gives you the grey levels where ghosting actually separates from the background, and it adds a moving pattern mode that helps prevent uneven ageing rather than only diagnosing it. Use the black and white screen tools for pixel faults and this one for retention and ghosting.',
  },
  {
    question: 'Does this apply to LCD/LED screens too, or only OLED/AMOLED?',
    answer: 'True burn-in is an OLED and AMOLED problem, because those panels age per pixel. LCD and LED screens can show image retention, a temporary ghost that usually clears by itself within minutes once the static image is gone, and both modes here work fine for spotting and clearing it. If a shadow on an LCD does not fade after a while, it is more likely a panel fault than retention.',
  },
];

const timerFaqs = () => [
  {
    question: 'Can I set a custom alarm sound?',
    answer: 'You can choose between two built-in alarms, a soft rising chime and a repeating beep, and switch the alarm off entirely if you need silence. Uploading your own audio file is not supported, because the alarms are generated in the browser rather than downloaded. If the room is loud or the device is muted, the screen also flashes when the countdown reaches zero, which is usually easier to notice than sound anyway.',
  },
  {
    question: 'Does the timer keep running if I switch to another browser tab?',
    answer: 'Yes. The countdown is worked out from a fixed end time rather than by counting ticks, so however long the tab sits in the background the remaining time is still correct the moment you come back. The one thing to know is that browsers deliberately slow down inactive tabs, so the alarm can fire a little late if the tab is hidden at the moment it hits zero. For anything where the exact second matters, leave this tab visible, ideally in fullscreen.',
  },
  {
    question: 'How is this different from the Pomodoro timer on the clock screen page?',
    answer: 'They solve different problems. This page is a general-purpose timer and stopwatch: any duration you like, quick presets, a custom entry, lap times, and a display sized to be read from the back of a room. The Pomodoro mode on the clock screen page is built around a focus routine instead, wrapping a countdown into the flip clock with a work interval and a custom reminder label. Use this page for talks, exams, cooking, and intervals; use the clock screen for structured focus sessions.',
  },
  {
    question: 'Is there a lap feature on the stopwatch?',
    answer: 'Yes. Switch to Stopwatch mode and the Lap button appears next to Start and Reset, or press L. Each lap is listed under the main display with both its total elapsed time and the split since the previous lap, newest first, so you can compare intervals without doing the subtraction yourself. Reset clears the list along with the clock.',
  },
  {
    question: 'Does this keep working if my internet connection drops after the page loads?',
    answer: 'Yes. Once the page has loaded, everything runs locally in your browser. The countdown, the stopwatch, the laps, and the alarm need no server, so a dropped connection mid-session changes nothing. That makes it dependable for a conference room with patchy wifi or a classroom where the network is unreliable. Reloading the page does need a connection, and it also clears the current timer.',
  },
];
...
export const tools: Tool[] = [
  ...
  { id: 'green-screen', name: 'Green Screen', shortName: 'Green', slug: 'green-screen', localizedSlugs: localizedColor('pantalla-verde', 'tela-verde', 'ecran-vert', 'gruener-bildschirm', 'hara-screen', 'midori-gamen'), category: 'colors', kind: 'color', color: '#00ff00', rgb: '0, 255, 0', summary: 'A free green screen online tool using the standard chroma key green (#00FF00) for video editing, streaming backgrounds, TikTok and social content, display testing, and photography lighting reference.', intent: 'the exact chroma key green (#00FF00) for clean video keying, streaming backgrounds, and display testing', useCases: ['chroma key video editing', 'streaming and video call backgrounds', 'TikTok and social media content creation', 'display panel testing', 'photography lighting reference'], controls: ['fullscreen', 'brightness', 'copy HEX', 'copy RGB', 'HEX input'], faqs: colorFaqs('Green Screen') },
  ...
  { id: 'dead-pixel-test', name: 'Dead Pixel Test', shortName: 'Dead Pixel', slug: 'dead-pixel-test', localizedSlugs: localizedColor('test-de-pixeles-muertos', 'teste-de-pixels-mortos', 'test-de-pixels-morts', 'pixelfehler-test', 'mrit-pixel-test', 'dotto-nuke-test'), category: 'colors', kind: 'test', color: '#808080', rgb: '128, 128, 128', summary: 'A guided six color test that cycles black, white, red, green, blue, and grey fullscreen to check any screen for dead pixels, stuck pixels, and backlight bleeding.', intent: 'a complete, repeatable inspection pass instead of a single solid color', useCases: ['new monitor and laptop checks', 'phone and tablet screen inspection', 'used device and resale checks', 'warranty claim documentation', 'sub-pixel channel isolation', 'backlight uniformity checks'], controls: ['fullscreen', 'next / previous', 'auto advance', 'jump to color', 'progress', 'color guidance', 'keyboard shortcuts'], faqs: deadPixelFaqs() },
  { id: 'oled-burn-in-test', name: 'OLED Burn-In Test', shortName: 'Burn-In', slug: 'oled-burn-in-test', localizedSlugs: {}, category: 'colors', kind: 'test', color: '#808080', rgb: '128, 128, 128', summary: 'A two mode OLED burn-in test: flat grey and color fills to reveal existing ghosting, plus a moving pattern that helps prevent uneven pixel ageing on OLED and AMOLED screens.', intent: 'both halves of the burn-in problem, checking for it and helping prevent it, on one screen', useCases: ['OLED and AMOLED phone checks', 'OLED TV and monitor inspection', 'used device and resale checks', 'idle screen protection', 'image retention checks on LCD', 'warranty claim documentation'], controls: ['fullscreen', 'static check', 'moving pattern', 'grey and color fills', 'brightness', 'pattern speed'], faqs: burnInFaqs() },
  ...
  { id: 'aquarium-screen', name: 'Aquarium Screen', shortName: 'Aquarium', slug: 'aquarium-screen', localizedSlugs: { es: 'pantalla-de-acuario', pt: 'tela-de-aquario', fr: 'ecran-aquarium', de: 'aquarium-bildschirm', hi: 'aquarium-screen', ja: 'aquarium-gamen' }, category: 'savers', kind: 'saver', color: 'linear-gradient(180deg, #003d66, #00bcd4)', summary: 'A free online aquarium screen tool to turn any display into a relaxing virtual fish tank with animated tropical fish, water bubbles, ocean soundscapes, custom color themes, and clock overlay.', intent: 'free virtual aquarium screen online for desktop, laptop, monitor, and Smart TV with animated tropical fish, bubbles, ocean soundscapes, and clock overlay', useCases: ['desktop & laptop ambience', 'TV aquarium background', 'office & study relaxation', 'café & waiting room displays', 'fullscreen screensaver alternative', 'second monitor decoration'], controls: ['fullscreen mode', 'water color themes', 'ocean soundscapes', 'fish count & speed', 'clock & timer overlay', 'custom bg music'], faqs: effectFaqs('Aquarium Screen') },
  ...
  { id: 'countdown-timer', name: 'Countdown Timer & Stopwatch', shortName: 'Timer', slug: 'countdown-timer', localizedSlugs: {}, category: 'savers', kind: 'saver', color: '#0b0b0f', summary: 'A fullscreen countdown timer and stopwatch with a display big enough to read from the back of a room, quick presets, a custom duration, selectable alarms, and lap times.', intent: 'a timer you can read from across the room rather than a widget you have to walk up to', useCases: ['presentations and conference talks', 'classroom activities and exams', 'workshops and timed exercises', 'cooking and kitchen timing', 'workouts and interval training', 'focus and study sessions'], controls: ['fullscreen', 'timer / stopwatch', 'presets', 'custom duration', 'alarm sounds', 'lap times', 'dark / light'], faqs: timerFaqs() },
  ...
  { id: 'ubuntu-screen', name: 'Ubuntu Update', shortName: 'Ubuntu', slug: 'ubuntu-screen', localizedSlugs: {}, category: 'fake', kind: 'fake', color: '#e95420', summary: 'A realistic fake Ubuntu update screen simulator with authentic boot animation, non-linear progress simulation, reboot cycles, and Ubuntu terminal aesthetic in realistic orange-and-purple brand colors. Free online tool for Linux demos, video production, classroom training, and harmless pranks.', intent: 'a convincing Ubuntu update and boot screen simulation for Linux demonstrations, educational training, video production, and harmless pranks', useCases: ['Linux demonstrations and training', 'YouTube and TikTok video production', 'streaming and OBS intermission screens', 'classroom and university Linux lectures', 'technology event displays and trade shows', 'theatre stage productions and escape rooms', 'developer environment backgrounds', 'screen recording and presentation backdrops'], controls: ['fullscreen', 'authentic boot animation', 'non-linear progress simulation', 'reboot cycle', 'brightness', 'timer', 'escape hint', 'auto-hide cursor'], faqs: effectFaqs('Ubuntu Update Screen') },
  { id: 'macos-screen', name: 'macOS Update', shortName: 'macOS', slug: 'macos-screen', localizedSlugs: {}, category: 'fake', kind: 'fake', color: '#1a1a1a', summary: 'A realistic fake macOS update screen simulator with 5 update scenarios (macOS Sequoia, Security Response, App Store, Firmware, Beta), 4 operational modes (Realistic, Timer, Manual, Infinite), and 3 themes including Dark Mode. Authentic Apple-style visuals with auto-restart animation and synthesized sounds. Free online tool for Apple UI demos, video production, and harmless pranks.', intent: 'a convincing macOS update and installation simulation for Apple UI demonstrations, video production, educational training, and harmless pranks', useCases: ['Apple UI demonstrations and mockups', 'YouTube and TikTok video production', 'streaming and OBS intermission screens', 'classroom and university macOS training', 'client presentations and software demos', 'theatre stage productions and escape rooms', 'technology event displays and trade shows', 'screen recording and presentation backdrops'], controls: ['fullscreen', '5 update scenarios', '4 operational modes', '3 visual themes', 'auto-restart animation', 'synthesized sounds', 'speed control', 'progress slider', 'timer duration', 'keyboard shortcuts', 'escape hint', 'auto-hide cursor'], faqs: effectFaqs('macOS Update') },
  { id: 'android-update', name: 'Android Update Screen', shortName: 'Android Update', slug: 'android-update', localizedSlugs: {}, category: 'fake', kind: 'fake', color: '#3ddc84', summary: 'A realistic fake Android system update screen simulator with authentic Android robot branding, configurable update duration (1min to 1hr), adjustable starting percentage, and two-phase restart animation with boot dots. Free online tool for mobile UI demos, video production, classroom training, and harmless pranks.', intent: 'a convincing Android system update simulation for mobile UI demonstrations, video production, educational training, and harmless pranks', useCases: ['mobile UI demonstrations and mockups', 'YouTube and TikTok video production', 'streaming and OBS intermission screens', 'classroom and university mobile OS training', 'technology event displays and trade shows', 'theatre stage productions and escape rooms', 'phone review and unboxing video backgrounds', 'screen recording and presentation backdrops'], controls: ['fullscreen', 'configurable update duration', 'adjustable start percentage', 'restart animation', 'animated boot dots', 'keyboard shortcuts', 'escape hint', 'auto-hide cursor'], faqs: effectFaqs('Android Update Screen') },
];
